            ...
            if match:
                ...
                if module_name in module_name_mapping:
                    ...
                    try:
                        ...
                        U = svd_data["U"]  # [out_features, min(out, in)]
                        S = svd_data["S"]  # [min(out, in)]
                        Vh = svd_data["Vh"]  # [min(out, in), in_features]
                        
                        # Get current module dimensions
                        in_features = module.lora_A.shape[-1]  # from [1, r, in_features]
                        out_features = module.lora_B.shape[1]  # from [B_num, out_features, r]
                        
                        # SVD: W = U @ diag(S) @ Vh
                        # For LoRA: W ≈ A @ B, where A is [r, in_features], B is [out_features, r]
                        # A = Vh[:r, :], B = U[:, :r] @ diag(S[:r])
                        # lora_A_init = Vh[:lora_r, :].to(module.lora_A.device).to(module.lora_A.dtype)  # [r, in_features]
                        S_diag = S[:lora_r].to(module.lora_B.device).to(module.lora_B.dtype)  # [r]
                        # lora_B_init = (U[:, :lora_r]).to(module.lora_B.device).to(module.lora_B.dtype)  # [out_features, r]
                        
                        # Initialize lora_A: [1, r, in_features]
                        with torch.no_grad():
                            # module.lora_A.data[0] = lora_A_init
                            module.lora_scale.data = S_diag
                            # Initialize lora_B: [B_num, out_features, r]
                            # Use the same initialization for all B matrices
...
def train(
    ...
    **kwargs,
    ...
    # Custom data collator to handle lambda_index
    class MLoRADataCollator(transformers.DataCollatorForSeq2Seq):
        def __call__(self, features, return_tensors=None):
            # Extract lambda_index if present
            lambda_indices = None
            ...
            # Add lambda_index to batch as tensor
            if lambda_indices is not None:
                # Ensure lambda_index values are within valid range [0, lambda_num)
    ...
    for name, param in model.named_parameters():
        ...
        logger.info(f"{name}: {param.size()}, requires_grad={param.requires_grad}")
    
    trainable_ratio = trainable_params / total_params if total_params > 0 else 0
    logger.info(f"总参数数量: {total_params:,}")
    logger.info(f"需要梯度的参数数量: {trainable_params:,}")
    logger.info(f"需要梯度的参数占比: {trainable_ratio:.4%}")
    ...
    trainer = transformers.Trainer(
        ...
        data_collator=MLoRADataCollator(
